...
class InputFeatures(object):
    def __init__(self, unique_id, tokens, input_type_ids, input_ids):
        self.unique_id = unique_id
        self.tokens = tokens
...
class BertMouthDataset(torch.utils.data.Dataset):
    def __init__(self, all_input_ids, all_input_mask, all_input_type_ids, all_num_tokens,
                 mask_id=4, ignore_index=-1):
        self.data_num = len(all_input_ids)
    ...
    def __getitem__(self, idx):
        ...
        target_token_ind = np.random.randint(1, num_tokens - 2)

        y = [self._ignore_index] * len(input_ids)
        y[target_token_ind] = input_ids[target_token_ind]
        y = torch.tensor(y, dtype=torch.long)
        input_ids[target_token_ind] = self._mask_id

        return input_ids, y, input_mask, input_type_ids
...
def convert_examples_to_features(examples, seq_length, tokenizer, ignore_index=-1):
    def make_feature(ex_index, tokens):
        ...
        if ex_index < 2:
            ...
            logger.info(
                "segment_ids: %s" % " ".join([str(x) for x in input_type_ids]))
